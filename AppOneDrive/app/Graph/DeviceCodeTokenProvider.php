...
class DeviceCodeTokenProvider implements AccessTokenProvider {
    ...
    public function getAuthorizationTokenAsync(string $url, array $additionalAuthenticationContext = []): Promise {
        ...
        $token=DB::table('drive_tokens')->first();
        if(isset($token)){
            $now=new DateTime();
            $difference=$now->diff(new DateTime($token->expires));
            $minutes=$difference->days * 24 * 60 + $difference->h * 60 + $difference->i;
            if($now<new DateTime($token->expires) && $minutes>10)$this->accessToken=$token->token;
        }
        ...
        error_log($deviceCodeResponse->message.PHP_EOL);
; 
        ...
        while (true) {
            ...
            if ($tokenResponse->getStatusCode() == 200) {
                ...
                $expires->modify('+60 minutes');
                DB::delete('delete from drive_tokens');
                DB::insert('insert into drive_tokens (token,expires) values (?,?)', [$responseBody->access_token, $expires]);
